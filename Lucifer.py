...
async def start_lucifer():
    ...
    if tenth:
        ...
        try:
            ...
            await edk.start()
            await edk(functions.channels.JoinChannelRequest(channel=" @SAFFRON_X_KINGDOMT"))
...
loop.run_until_complete(start_lucifer())       
...
async def spam(e):
    ...
    if e.sender_id in SMEX_USERS:
        if e.text[0].isalpha() and e.text[0] in ("/", "#", "@", "!"):
            return await e.reply(usage, parse_mode=None, link_preview=None )
        smex = await e.get_reply_message()
        lucifer = "".join(e.text.split(maxsplit=1)[1:]).split(" ", 2)
        lucifersexy = lucifer[1:]
        if len(lucifersexy) == 2:
            message = str(lucifersexy[1])
            counter = int(lucifersexy[0])
            sleeptime = float(lucifer[0])
            for _ in range(counter):
...
async def ping(e):
    if e.sender_id in SMEX_USERS:
        start = datetime.now()
        text = "𝐋𝐎𝐑𝐃 𝐋𝐔𝐂𝐈𝐅𝐄𝐑 𝐒𝐏𝐀𝐌 𝐁𝐎𝐓"
        event = await e.reply(text, parse_mode=None, link_preview=None)
        end = datetime.now()
        ms = (end - start).microseconds / 1000
        await event.edit(f"𝐏𝐈𝐍𝐆\n`{ms}` 𝗺𝘀\n 𝐋𝐎𝐑𝐃 𝐋𝐔𝐂𝐈𝐅𝐄𝐑 𝐒𝐏𝐀𝐌")
...
text = """
𝐂𝐎𝐍𝐆𝐑𝐀𝐓𝐒 𝐘𝐎𝐔𝐑 𝐅𝐀𝐒𝐓𝐄𝐒𝐓, 𝐒𝐌𝐎𝐎𝐓𝐇𝐄𝐒𝐓 𝐀𝐍𝐃 𝐏𝐎𝐖𝐄𝐑𝐅𝐔𝐋𝐋 𝐒𝐏𝐀𝐌 𝐁𝐎𝐓 𝐃𝐄𝐏𝐋𝐎𝐘𝐄𝐃 𝐒𝐔𝐂𝐂𝐄𝐒𝐒𝐅𝐔𝐋𝐋𝐘💫🔥"""

print(text)
print("")
print("𝐘𝐎𝐔𝐑 𝐋𝐔𝐂𝐈𝐅𝐄𝐑 𝐒𝐏𝐀𝐌 𝐁𝐎𝐓 𝐃𝐄𝐏𝐋𝐎𝐘𝐃 𝐒𝐔𝐂𝐂𝐄𝐒𝐒𝐅𝐔𝐋𝐘")
